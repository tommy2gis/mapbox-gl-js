...
class RasterTileSource extends Evented implements Source {
    ...
    loadTile(tile: Tile, callback: Callback<void>) {
        ...
        const url = this.map._requestManager.normalizeTileURL(tile.tileID.canonical.url(this.tiles, this.scheme, this.zoomOffset,this.map.crs), use2x, this.tileSize);
        tile.request = getImage(this.map._requestManager.transformRequest(url, ResourceType.Tile), (err, img) => {
            ...
            if (tile.aborted) {
                ...
                if (tile.texture) {
                    ...
                    var filterVal = this.map.isIntScrollZoom===true ? gl.NEAREST : gl.LINEAR;
                    tile.texture.bind(filterVal, gl.CLAMP_TO_EDGE, gl.LINEAR_MIPMAP_NEAREST);
